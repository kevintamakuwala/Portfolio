...
export function ContactForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!email || !isEmail(email)) {
      ...
      setInvalid({ ...invalid, message: true });
    ...
    try {
      ...
      setInvalid({ email: false, name: false, message: false });
  ...
  const handleChange = (
    ...
    if (type === 'message' && e.target.value.trim().length > 500) {
      ...
      setInvalid({ ...invalid, message: false });
    }
  ...
  return (
    ...
    >
      <div className="block w-full items-center justify-center gap-4 text-left md:flex">
        <label
          htmlFor="name"
          className="w-full text-left text-sm font-semibold tracking-wide text-neutral-700 dark:text-neutral-300"
        >
          Name
          <span
            aria-hidden="true"
            ...
            className={cn(
              {
                ...
                'border-neutral-300 text-neutral-800 focus:border-blue-600 dark:border-neutral-800 dark:text-white focus:dark:border-neutral-700':
                  !invalid.name,
              },
